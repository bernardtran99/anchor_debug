        ...
        if ("On Interest" in message) or ("Flooded Interest" in message):
            for i in range(len(strings)):
                ...
                if strings[i] == "Interest:":
                    ...
                    global G
                    G.add_edges_from([(string_value, node_num)],color='black')
                if (strings[i] == "Flooded") and (node_num != 1) and ((node_num, firstInterest[node_num]) not in input_layer2_list):
                    ...
                    G.add_edges_from([(node_num, firstInterest[node_num])], color='blue')
        if "Data" in message:
            ...
            if "On Data: 8" in message:
                if (prev_node8, node_num) != (1,1) :
                    G.add_edges_from([(prev_node8, node_num)],color='black')
            ...
            if "On Data: 9" in message:
                ...
                node9_list.append((prev_node9, node_num),color='black')
                prev_node9 = node_num
