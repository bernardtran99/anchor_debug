...
ndn_udp_face_t *generate_udp_face(char* input_ip, char *port_1, char *port_2) {
    ...
    for(int i = 0; i < 5; i++) {
        ...
        check_port_1 = sprintf(check_port_1, "%c", htons(udp_table.faces[i]->remote_addr.sin_port));
        printf("PORT1: %s, ", check_port_1);
        check_port_2 = sprintf(check_port_1, "%c", htons(udp_table.faces[i]->local_addr.sin_port));
        printf("PORT2: %s\n", check_port_2);
